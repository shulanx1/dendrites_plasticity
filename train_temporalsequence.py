...
num_patterns = 3
seed = 1		 			# random seed
file_name_identifier = '20ms_basal_only_3features'
model_file = wd + '\\outputs\\example_time_sequence_' + file_name_identifier  # for saving output
path = wd + '\\outputs'
...
def init_input_timesequence(P, num_patterns, stim_on, stim_dur, r_mean, r_max, num_t, s, deltaT = 20):
    ...
    rates_e, rates_i = sequences.assoc_rates_time_squence(num_patterns, N_e, N_i, r_mean,
                                             r_max)
    rates_e = [r[ind_e] for r in rates_e]
    rates_i = [r[ind_i] for r in rates_i]
    if s > 0:
        S_E, S_I = sequences.assoc_seqs_time_sequence(num_patterns, N_e, N_i, stim_on, stim_on+stim_dur,
                                        num_t, deltaT)
        ...
        S_E, S_I = sequences.build_seqs(num_patterns, N_e, N_i, stim_on, stim_on+stim_dur,
                                        0)
...
def init_labels_time_sequence(num_patterns):
    """
    ...
    num = len(permutation_label)
    if num > 2:
        L = sequences.assign_labels(num)
    else:
...
if model == 'pas':
    ...
    P['g_max_A'] /= 5
    P['g_max_N'] /= 5
    kernel_file = wd + '\\input\\kernel_fit_pas1'
elif model == 'pn':
    P['locs_e'] = np.array(P['soma'])
    P['locs_i'] = np.array(P['soma'])
    kernel_file = wd + '\\input\\kernel_fit_soma1'
else:
    kernel_file = wd + '\\input\\kernel_fit_act1'
    _, kernel = pickle.load(open(kernel_file, 'rb'))
#     kernel_file = wd + '\\input\\kernel_fit_pas1'
#     kernel = pickle.load(open(kernel_file, 'rb'))
np.random.seed(seed)
rates_e, rates_i, S_E, S_I, = init_input_timesequence(P, num_patterns, stim_on, stim_dur,
                                        r_mean, r_max, num_t, s, deltaT)
Labels = init_labels_time_sequence(num_patterns)
w_e, w_i = init_weights(P)
num_sequences = len(Labels)
E = [num_sequences/2]
E_P = [[0.5] for k in range(num_sequences)]
W_E = [np.array(w_e)]
W_I = [np.array(w_i)]

### Test inital Model ####
test_file_init = wd + "\\outputs\\test_init_" + file_name_identifier
...
V = np.array(V)
pickle.dump([V, rates_e, rates_i, S_e_all, S_i_all,Labels,t],open(test_file_init, 'wb'))
...
E, E_P, W_E, W_I, P = training.train_L5(rates_e, rates_i, S_E, S_I, Labels, E, E_P,
                                    W_E, W_I, P, kernel)
E = E[1:]
E_sm = np.convolve(E, np.ones(20)/20, mode='valid')/num_sequences
if save_output:
...
### Test Trained Model ####
test_file = wd + "\\outputs\\test_trained_timesequence_" + file_name_identifier
h('forall pop_section()')
h('forall delete_section()')
reps = 5
cell = neuron_model.NModel(P)
