...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 3, "cvodeieq");
 	...
 	ivoc_help("help ?1 NaTa_t_2F C:/work/Code/Dendrites_plasticity/mod_stochastic/NaTa_t_2F.mod\n");
...
static int  rates (  ) {
 ...
 _lqt = pow( 2.3 , ( ( 34.0 - 21.0 ) / 10.0 ) ) ;
   ...
   SDm = sqrt ( fabs ( mAlpha * ( 1.0 - m ) + mBeta * m ) / ( 0.05 * NNaTa_t * 3.0 ) ) ;
   if ( v  == - 66.0 ) {
     v = v + 0.0001 ;
     }
   ...
   SDh = sqrt ( fabs ( hAlpha * ( 1.0 - h ) + hBeta * h ) / ( 0.05 * NNaTa_t ) ) ;
     return 0; }
